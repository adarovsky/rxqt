...
/*! \file rx-drop_map.hpp
    ...
    \tparam CollectionSelector  the type of the observable producing function. CollectionSelector must be a function with the signature: observable(drop_map::source_value_type)
    \tparam ResultSelector      the type of the aggregation function (optional). ResultSelector must be a function with the signature: drop_map::value_type(drop_map::source_value_type, drop_map::collection_value_type)
    ...
    Observables, produced by the CollectionSelector, are drop_mapenated. There is another operator rxcpp::observable<T,SourceType>::flat_map that works similar but merges the observables.

    \sample
    \snippet drop_map.cpp drop_map sample
    \snippet output.txt drop_map sample

    \sample
    \snippet drop_map.cpp threaded drop_map sample
    \snippet output.txt threaded drop_map sample
*/

#if !defined(RXCPP_OPERATORS_RX_DROPMAP_HPP)
#define RXCPP_OPERATORS_RX_DROPMAP_HPP
...
template<class... AN>
struct drop_map_invalid_arguments {};

template<class... AN>
struct drop_map_invalid : public rxo::operator_base<drop_map_invalid_arguments<AN...>> {
    using type = observable<drop_map_invalid_arguments<AN...>, drop_map_invalid<AN...>>;
};
template<class... AN>
using drop_map_invalid_t = typename drop_map_invalid<AN...>::type;

template<class Observable, class CollectionSelector, class ResultSelector, class Coordination>
struct drop_map_traits {
    ...
    static_assert(!std::is_same<decltype(collection_check<source_value_type, collection_selector_type>(0)), tag_not_valid>::value, "drop_map CollectionSelector must be a function with the signature observable(drop_map::source_value_type)");

    typedef decltype((*(collection_selector_type*)nullptr)((*(source_value_type*)nullptr))) collection_type;

//#if _MSC_VER >= 1900
    static_assert(is_observable<collection_type>::value, "drop_map CollectionSelector must return an observable");
//#endif
    ...
    static_assert(!std::is_same<decltype(result_check<source_value_type, collection_value_type, result_selector_type>(0)), tag_not_valid>::value, "drop_map ResultSelector must be a function with the signature drop_map::value_type(drop_map::source_value_type, drop_map::collection_value_type)");
...
template<class Observable, class CollectionSelector, class ResultSelector, class Coordination>
struct drop_map
    : public operator_base<rxu::value_type_t<drop_map_traits<Observable, CollectionSelector, ResultSelector, Coordination>>>
{
    typedef drop_map<Observable, CollectionSelector, ResultSelector, Coordination> this_type;
    typedef drop_map_traits<Observable, CollectionSelector, ResultSelector, Coordination> traits;
    ...
    drop_map(source_type o, collection_selector_type s, result_selector_type rs, coordination_type sf)
    ...
    void on_subscribe(Subscriber scbr) const {
        ...
        struct drop_map_state_type
            : public std::enable_shared_from_this<drop_map_state_type>
            , public values
        {
            drop_map_state_type(values i, coordinator_type coor, output_type oarg)
                : values(std::move(i))
            ...
            void subscribe_to(source_value_type st)
            {
                ...
                //on_completed
                    [state](){
                        if (!state->lastValue.empty()) {
                            auto value = state->lastValue.get();
                            state->lastValue.reset();
            ...
            rxu::detail::maybe<source_value_type> lastValue;
        ...
        auto state = std::make_shared<drop_map_state_type>(initial, std::move(coordinator), std::move(scbr));
        ...
        // on_next
            [state](source_value_type st) {
                if (state->collectionLifetime.is_subscribed()) {
                    state->lastValue.reset(st);
                } else if (state->lastValue.empty()) {
        ...
        // on_completed
            [state]() {
                if (!state->collectionLifetime.is_subscribed() && state->lastValue.empty()) {
                    state->out.on_completed();
                }
            }
        );
...
private:
    drop_map& operator=(const drop_map&) RXCPP_DELETE;
};

}

/*! @copydoc rx-drop_map.hpp
*/
template<class... AN>
auto drop_map(AN&&... an)
->     operator_factory<drop_map_tag, AN...> {
    return operator_factory<drop_map_tag, AN...>(std::make_tuple(std::forward<AN>(an)...));
}

}

template<>
struct member_overload<drop_map_tag>
{
    template<class Observable, class CollectionSelector,
        ...
        class DropMap = rxo::detail::drop_map<rxu::decay_t<Observable>, rxu::decay_t<CollectionSelector>, ResultSelectorType, identity_one_worker>,
        ...
        class Result = observable<Value, DropMap>
    >
    static Result member(Observable&& o, CollectionSelector&& s) {
        return Result(DropMap(std::forward<Observable>(o), std::forward<CollectionSelector>(s), ResultSelectorType(), identity_current_thread()));
    }
    ...
    template<class Observable, class CollectionSelector, class Coordination,
        ...
        class DropMap = rxo::detail::drop_map<rxu::decay_t<Observable>, rxu::decay_t<CollectionSelector>, ResultSelectorType, rxu::decay_t<Coordination>>,
        class CollectionValueType = rxu::value_type_t<CollectionType>,
        class Value = rxu::result_of_t<ResultSelectorType(SourceValue, CollectionValueType)>,
        class Result = observable<Value, DropMap>
    >
    static Result member(Observable&& o, CollectionSelector&& s, Coordination&& cn) {
        return Result(DropMap(std::forward<Observable>(o), std::forward<CollectionSelector>(s), ResultSelectorType(), std::forward<Coordination>(cn)));
    }
    ...
    template<class Observable, class CollectionSelector, class ResultSelector,
        ...
        class DropMap = rxo::detail::drop_map<rxu::decay_t<Observable>, rxu::decay_t<CollectionSelector>, rxu::decay_t<ResultSelector>, identity_one_worker>,
        class CollectionValueType = rxu::value_type_t<CollectionType>,
    ...
    static Result member(Observable&& o, CollectionSelector&& s, ResultSelector&& rs) {
        return Result(DropMap(std::forward<Observable>(o), std::forward<CollectionSelector>(s), std::forward<ResultSelector>(rs), identity_current_thread()));
    }
    ...
    template<class Observable, class CollectionSelector, class ResultSelector, class Coordination,
        ...
        class Enabled = rxu::enable_if_all_true_type_t<
            all_observables<Observable, CollectionType>,
            is_coordination<Coordination>>,
        class DropMap = rxo::detail::drop_map<rxu::decay_t<Observable>, rxu::decay_t<CollectionSelector>, rxu::decay_t<ResultSelector>, rxu::decay_t<Coordination>>,
        class CollectionValueType = rxu::value_type_t<CollectionType>,
        class ResultSelectorType = rxu::decay_t<ResultSelector>,
        class Value = rxu::result_of_t<ResultSelectorType(SourceValue, CollectionValueType)>,
        class Result = observable<Value, DropMap>
    ...
    static Result member(Observable&& o, CollectionSelector&& s, ResultSelector&& rs, Coordination&& cn) {
        return Result(DropMap(std::forward<Observable>(o), std::forward<CollectionSelector>(s), std::forward<ResultSelector>(rs), std::forward<Coordination>(cn)));
    }

    template<class... AN>
    static operators::detail::drop_map_invalid_t<AN...> member(AN...) {
        std::terminate();
        return {};
        static_assert(sizeof...(AN) == 10000, "drop_map takes (CollectionSelector, optional ResultSelector, optional Coordination)");
    }
};
